...
@section('modals')
    <x-modal id="modal-create" title="Tambah Rate Bank">
        <x-form id="form-create" method="post">
            <x-form-select label="Bank" id="create-bank-id" :options="[]" name="bankId" class="mb-3" required/>
    ...
    <x-modal id="modal-edit" title="Ubah Rate Bank">
        <x-form id="form-edit" method="put">
            <x-form-select label="Bank" id="edit-bank-id" :options="[]" name="bankId" class="mb-3" required/>
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#create-bank-id, #create-insurance-id, #create-insurance-type-id").select2({dropdownParent: $('#modal-create')})
            $("#edit-bank-id, #edit-insurance-id, #edit-insurance-type-id").select2({dropdownParent: $('#modal-edit')})
        })

        $(document).on('click', '.btn-delete', function () {
            // Delete
            NegativeConfirm.fire({
                title: "Yakin ingin menghapus Rate Bank?",
            }).then((result) => {
