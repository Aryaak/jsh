...
@push('js')
    ...
    <script>
        ...
        $(document).on('click', '.btn-show', function () {
            ajaxGet("{{ route('master.bank-rates.show','-id-') }}".replace('-id-',$(this).data('id')),'',function(response){
                if(response.success){
                    ...
                    $('#show-min-value').html((ToRupiah.format(bankRate.min_value)).replace('\u00A0', '') + ",-")
                    $('#show-rate-value').html(ToUnit.format(bankRate.rate_value))
                    $('#show-polish-cost').html((ToRupiah.format(bankRate.polish_cost)).replace('\u00A0', '') + ",-")
                    $('#show-stamp-cost').html((ToRupiah.format(bankRate.stamp_cost)).replace('\u00A0', '') + ",-")
        ...
        $(document).on('click', '.btn-edit', function () {
            ...
            $('#edit-min-value').val((ToRupiah.format(bankRate.min_value)).replace('Rp\u00A0', ''))
            $('#edit-rate-value').val(ToUnit.format(bankRate.rate_value))
            $('#edit-polish-cost').val((ToRupiah.format(bankRate.polish_cost)).replace('Rp\u00A0', ''))
            $('#edit-stamp-cost').val((ToRupiah.format(bankRate.stamp_cost)).replace('Rp\u00A0', ''))
